...
function UsersPage() {
  ...
  useEffect(() => {
    ...
    const getdata = async () => {
      ...
      try {
        ...
        const response = await axiosInstance.get(`/users/getKomitmen?id_user=${id_user}&is_active=1&bulanBuat=${filterBulan}&tahunBuat=${filterTahun}`);
  ...
  }, [id_admin, namaUnit, token, filterTahun, filterBulan]);
          ...
          <div style={styles.conten}>
            <Container maxWidth="xl">
              <Grid container spacing={3}>
                ...
                <Grid>
                {lock ? <Button onClick={tambahRealisasi}>Tambah Komitmen</Button> : null}
                </Grid>
                <Grid >
                        ...
                        {data
                          ...
                          .map((row, index) => (
                            <TableRow key={index}>
                              ...
                              <TableCell>Rp. {row.KomitmenAnggaranTahunBerjalan.toLocaleString()}</TableCell>
